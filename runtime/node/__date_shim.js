// JSBin Runtime - Date 本地化格式 shim(toLocaleString / toLocaleDateString /
...
// jsbin 的 Date 无时区(getTimezoneOffset()==0):由本地分量构造的 Date 与 node 一致;
// 由裸时间戳构造的 Date(如 new Date(0))node 按机器时区、jsbin 按 UTC —— 记文档偏差。
...
// (jsbin 调用约定仅 6 个参数寄存器,7 参会丢第 7 个)。
...
// toDateString:"Www Mon DD YYYY"(node 用本地日期;jsbin 无时区按 UTC → 本地分量构造的
