...
A self-hosting, zero-dependency JavaScript→native AOT compiler (5 targets: macOS/Linux arm64+x64, Windows x64). Latest release **v1.5.52** — **G-M-P N>2: 3+ OS threads** — generalized work-stealing + an N-way stop-the-world GC across 3 real threads (linux-arm64); `GOMAXPROCS=1` stays a byte-identical fixed point — on top of NUL-transparent strings, AES-GCM crypto, x64 `process.exit`/`Date` fixes, the test262 harness (20.4%), real zlib / TCP, full compiler determinism (`gen1==gen2==gen3`), and full async. See **[CHANGELOG.md](./CHANGELOG.md)** for the full version history.
...
- Node-style builtins: `console`, `process`, `fs` (partial), `path`, `timers` (partial), `os`; plus — landed v1.5.42–v1.5.51, all partial subsets — real `crypto` (SHA-1/256/512, HMAC, AES-CBC/CTR/GCM, PBKDF2, HKDF), `zlib` (DEFLATE/gzip), `net`/`http`/`dgram`, `stream`, `child_process`; per-module status: [docs/NODEJS_SUPPORT_ANALYSIS.md](./docs/NODEJS_SUPPORT_ANALYSIS.md)
...
- **Pre-production.** No stability guarantees, no semver discipline yet, one primary developer. Test coverage is primarily fixture-based (362/362); a test262 conformance harness landed in v1.5.51 — first baseline 1,318 / 6,462 = 20.4% on a stride-5 subset (`language/` + 13 core `built-ins/`, see `tests/test262/last_report.md`) — a low number that is being actively raised.
