...
// asmjs_eval(source[, options]) -> JSValue
// 间接 eval 语义(全局作用域)。直接 eval 的词法捕获属 P4 后续。
export function asmjsEval(source, options) {
    options = options || {};
    ...
    throw new Error("asm.js engine P0-P2 未实现:见 engine/README.md 阶段规划");
}

// asmjs_function(argNames, body) -> 可调用函数(new Function 语义)。
// 编译 `function(argNames){ body }` 片段,缓存编译结果(同体复用)。
export function asmjsFunction(argNames, body) {
    // TODO(P4):合成函数体源码,走 asmjsEval 的片段编译,返回宿主可调用闭包。
    throw new Error("asm.js engine new Function 未实现:见 engine/README.md 阶段规划");
}

// asmjs_import(runtimeSpecifier) -> Promise<namespace>
// 运行时 specifier 的动态 import(静态 specifier 已在主编译器 AOT 完成,非本引擎)。
export function asmjsImport(specifier) {
    // TODO(P4):运行时解析 + 编译目标模块成片段 + 载入模块表 + resolved Promise。
    throw new Error("asm.js engine 运行时 import() 未实现:见 engine/README.md 阶段规划");
}
